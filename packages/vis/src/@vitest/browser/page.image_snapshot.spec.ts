...
it('save file under __results__', async ({ task }) => {
	...
	expect(f1.resultPath).toMatch(`../../../__snapshots__/__results__/${filename}/${toSnapshotId(task.name)}-1.png`)

	const f2 = await page.imageSnapshot()
	expect(f2.resultPath).toMatch(`../../../__snapshots__/__results__/${filename}/${toSnapshotId(task.name)}-2.png`)
})
