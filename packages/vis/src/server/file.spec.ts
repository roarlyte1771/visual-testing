...
describe(`${file.tryReadBase64.name}()`, () => {
	it('returns base64 encoded content when file exists', async () => {
		...
		const result = await file.tryReadBase64('existing-file.txt')
	...
	it('returns undefined when file does not exist', async () => {
		...
		const result = await file.tryReadBase64('non-existent-file.txt')
	...
	it('returns undefined for any file system error', async () => {
		...
		const result = await file.tryReadBase64('inaccessible-file.txt')
		expect(result).toBeUndefined()
	...
	it('handles empty files correctly', async () => {
		...
		const result = await file.tryReadBase64('empty-file.txt')
		expect(result).toBe('')
